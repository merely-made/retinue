"""Capture stock LXMF opportunistic delivery through Retinue's ratchet receiver.

Only public LXMF and RNS APIs are called. The Python implementation source is
not read. The Rust process prints the decrypted link-less packet before an
Outrider opportunistic API exists.
"""
...
def main() -> int:
    ...
    config = Path(tempfile.mkdtemp(prefix="outrider-opportunistic-capture-"))
    ...
    try:
        ...
        print(f"captured ordinary signed LXMF object: {'PASS' if title_ok and content_ok else 'FAIL'}")
        print(f"stock and Outrider message ids agree: {'PASS' if captured_id == message_hash else 'FAIL'}")
        print(f"STOCK_OPPORTUNISTIC_CAPTURE: {'PASS' if ok else 'FAIL'}")
