...
| O4–O6 (observations) | O4 active-USB emission landed in `bad58a0`; `c0adcbf` supplies the missing bounded USB reply carrier and retires a stalled host stream until external reset while RF continues. Automated checks pass; physical acceptance pending. O5 durable capture landed in `ca32e0e` with 20 focused tests; headed view and settings remain open. Full O4 still needs a low-power observation path. O6 needs a validated second listener, including a V4 adapter with the current one-T114 bench. | Same |
